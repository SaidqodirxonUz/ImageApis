...
const api2 = async (req, res) => {
  try {
    ...
    const image = await loadImage(
      path.join(__dirname, "../../img/img2/photo.jpg")
    );
    ...
    if (text.length === 6 || text.length === 7 || text.length === 8) {
      ctx.fillText(text, olchamX - 20, olchamY);
      console.log("2 iffda");
    }
    ...
    fs.writeFileSync(
      path.join(__dirname, "../../public/2/goto.png"),
      buffer,
      (err) => {
        if (err) {
    ...
    res.send(`
    <title>New Year Api 2</title>
    <center>    
    <img src="${mainUrl}/2/goto.png" alt="Image with Text" />
    <br/>
    <br/>
    <p>Sizning rasmingiz</p>
    <p>Apidan foydalanish uchun 
    <br/> 
    <code>${mainUrl}/api/2</code>   <br/> 
    ga GET sorivini jonatib 
    <br/> 
    <code>${mainUrl}/2/goto.png </code>
    <br/> 
